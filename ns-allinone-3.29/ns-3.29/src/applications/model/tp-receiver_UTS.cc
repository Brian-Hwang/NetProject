...
namespace ns3
{
    ...
    NS_LOG_COMPONENT_DEFINE("TPReceiverUTS");

    NS_OBJECT_ENSURE_REGISTERED(TPReceiverUTS);

    TypeId TPReceiverUTS::GetTypeId(void)
    {
        static TypeId tid = TypeId("ns3::TPReceiverUTS")
                                .SetParent<Application>()
                                .AddConstructor<TPReceiverUTS>()
                                .AddAttribute("Local", "The Address on which to Bind the RX socket.",
                                              AddressValue(),
                                              MakeAddressAccessor(&TPReceiverUTS::m_address),
                                              MakeAddressChecker())
                                .AddAttribute("InFile", "The name of the input file to get data for calculation from.",
                                              StringValue(),
                                              MakeStringAccessor(&TPReceiverUTS::m_inFilename),
                                              MakeStringChecker())
                                .AddAttribute("OutFile", "The name of the output file to write received data to.",
                                              StringValue(),
                                              MakeStringAccessor(&TPReceiverUTS::m_outFilename),
                                              MakeStringChecker())
                                .AddAttribute("GameSize", "Size of the displayed field in game.",
                                              UintegerValue(),
                                              MakeUintegerAccessor(&TPReceiverUTS::m_fieldSize),
                                              MakeUintegerChecker<uint8_t>())
                                .AddAttribute("DisplayFreq", "Time between displaying different frames. (FPS maybe?)",
                                              TimeValue(Seconds(0.1)),
                                              MakeTimeAccessor(&TPReceiverUTS::m_dispFreq),
                                              MakeTimeChecker())
                                .AddAttribute("FileIO", "Determines whether the application runs its own frame generation, or if it gets frames over File IO",
                                              BooleanValue(false),
                                              MakeBooleanAccessor(&TPReceiverUTS::m_fileIO),
                                              MakeBooleanChecker())
                                .AddTraceSource("Rx", "A packet has been received",
                                                MakeTraceSourceAccessor(&TPReceiverUTS::m_rxTrace), "ns3::Packet::TracedCallback")
                                .AddTraceSource("Tx", "A packet has been sent",
                                                MakeTraceSourceAccessor(&TPReceiverUTS::m_txTrace), "ns3::Packet::TracedCallback");
        return tid;
    }

    TPReceiverUTS::TPReceiverUTS()
        : m_socket(0),
          m_totalRx(0),
          m_running(false),
          m_currPos(0),
    ...
    void TPReceiverUTS::StartApplication(void)
    {
        NS_LOG_FUNCTION(this);
        ...
        if (!m_socket)
        {
            ...
            m_socket->SetRecvCallback(MakeCallback(&TPReceiverUTS::HandleRead, this));
        }
    ...
    void TPReceiverUTS::ScheduleDisplay(void)
    {
        NS_LOG_FUNCTION(this);
        if (m_running)
        {
            m_displayEvent = Simulator::Schedule(m_dispFreq, &TPReceiverUTS::Display, this);
        }
    }

    char *TPReceiverUTS::NextFrame(uint16_t dim)
    {
        NS_LOG_FUNCTION(this);
    ...
    void TPReceiverUTS::Display(void)
    {
        NS_LOG_FUNCTION(this);
    ...
    void TPReceiverUTS::HandleRead(Ptr<Socket> socket)
    {
        NS_LOG_FUNCTION(this);
        Ptr<Packet> packet;
        Address from;
    ...
    void TPReceiverUTS::StopApplication()
    {
        NS_LOG_FUNCTION(this);
        m_running = false;
