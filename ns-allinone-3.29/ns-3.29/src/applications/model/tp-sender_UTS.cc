...
#include "ns3/tp-sender_UTS.h"
#include "ns3/string.h"

namespace ns3
{

    NS_LOG_COMPONENT_DEFINE("TPSender_UTS");

    NS_OBJECT_ENSURE_REGISTERED(TPSender_UTS);

    TypeId TPSender_UTS::GetTypeId(void)
    {
        static TypeId tid = TypeId("ns3::TPSender_UTS")
                                .SetParent<Application>()
                                .AddConstructor<TPSender_UTS>()
                                .AddAttribute("Address", "The address",
                                              AddressValue(),
                                              MakeAddressAccessor(&TPSender_UTS::m_address),
                                              MakeAddressChecker())
                                .AddAttribute("NPackets", "The total number of packets to send",
                                              UintegerValue(10000),
                                              MakeUintegerAccessor(&TPSender_UTS::m_nPackets),
                                              MakeUintegerChecker<uint32_t>())
                                .AddAttribute("FileName", "The name of the file to be transmitted.",
                                              StringValue(),
                                              MakeStringAccessor(&TPSender_UTS::m_filename),
                                              MakeStringChecker())
                                .AddAttribute("DataRate", "The data rate",
                                              DataRateValue(DataRate("500kb/s")),
                                              MakeDataRateAccessor(&TPSender_UTS::m_dataRate),
                                              MakeDataRateChecker())
                                .AddTraceSource("Tx", "A new packet is created and is sent",
                                                MakeTraceSourceAccessor(&TPSender_UTS::m_txTrace), "ns3::Packet::TracedCallback");
        return tid;
    }

    TPSender_UTS::TPSender_UTS()
        : m_socket(0),
          m_packetSize(1000),
          m_packetsSent(0),
          m_running(false),
    ...
    void TPSender_UTS::StartApplication(void)
    {
        NS_LOG_FUNCTION(this);
    ...
    void TPSender_UTS::SendPacket(void)
    {
        NS_LOG_FUNCTION(this);
    ...
    void TPSender_UTS::ScheduleTx(void)
    {
        NS_LOG_FUNCTION(this);
        if (m_running)
        {
            ...
            m_sendEvent = Simulator::Schedule(tNext, &TPSender_UTS::SendPacket, this);
        }
    }

    void TPSender_UTS::StopApplication()
    {
        NS_LOG_FUNCTION(this);
        m_running = false;
